...
def Reference_white(image):
    ...
    for i in range(0, image_rows):
        for j in range(0, image_cols):
            ...
            gray = int(
                (image[i, j, 0] * 114 + image[i, j, 1] * 587 + image[i, j, 2] * 299) / 1000)
            grayImg[i, j] = gray
            # 灰度值（每个亮度值）统计
            histogram[gray] += 1
...
def cal_black_pixel(eyeMat):
    num = 0
    # 高
    for i in range(eyeMat.shape[0]):
        # 宽
        for j in range(eyeMat.shape[0]):
            if eyeMat[i, j] == 0:
                num += 1
    return num
######## end ##############
...
def judgeFatigueState(dA):
    ...
    # 计算闭眼帧数占比
    dA.ECR = (1.0 * dA.closeFrame) / dA.totalFrame
    print "闭眼持续最长时间maxECT：{}".format(dA.maxECT)
    print "闭眼眼睛闭合比ECR：{}".format(dA.ECR)
    print "眨眼频率BF：{}".format(dA.BF)
    print "点头频率NF：{}".format(dA.NF)
    print "打哈欠频率YF：{}".format(dA.YF)
    print "最长张嘴时间maxYT:{}".format(dA.maxYT)
    ...
    with open("f:/test.txt", "a") as f:
        f.write("闭眼帧数：{}".format(dA.closeFrame)+"\n")
        f.write("闭眼持续最长时间maxECT：{}".format(dA.maxECT)+"\n")
        f.write("闭眼眼睛闭合比ECR：{}".format(dA.ECR)+"\n")
        f.write("眨眼频率BF：{}".format(dA.BF)+"\n")
        f.write("点头频率NF：{}".format(dA.NF)+"\n")
        f.write("打哈欠频率YF：{}".format(dA.YF)+"\n")
        f.write("最长张嘴时间maxYT:{}".format(dA.maxYT)+"\n")
        f.write("疲劳值：{}".format(dA.fatigueRation)+"\n\n")

        if dA.fatigueRation >= 0.3 and dA.fatigueRation < 0.7:
            f.write("您已处于疲劳状态，请尽快停车休息！"+"\n\n")
        if dA.fatigueRation >= 0.7:
            f.write("您已严重疲劳，请立即停车休息"+"\n\n")
...
if __name__ == '__main__':
    ...
    # 设置帧率
    vs.set(cv2.cv.CV_CAP_PROP_FPS, fps)
#     fps =vs.get(cv2.cv.CV_CAP_PROP_FPS) #获取视频帧数
#     print 'fps:',fps
    ...
    while True:
        ...
        if not paused:
            ...
            # 已经锁定到对象进行跟踪
                ...
                endY = int(pos.bottom())
                # 若丢失跟踪目标 重新检测人脸
                if is_found < 10 or startX < 0 or startY < 0 or endX < 0 or endY < 0:
                    tracker = None
                    errorFrameNum += 1
                    totalTime += time.time() - runTime
                    continue
                ...
                # print "人脸特征点定位时间：{}".format(time.time()-t1)
                ...
                # 绘制左眼边框
                stratX = int(
                    leftEye[0][0] - abs(leftEye[0][0] - leftEye[3][0]) * 0.3)
                stratY = int(
                    min(leftEye[0][1], leftEye[3][1]) - abs(leftEye[1][1] - leftEye[5][1]) * 1.5)
                endX = int(
                    leftEye[3][0] + abs(leftEye[0][0] - leftEye[3][0]) * 0.3)
                endY = int(
                    min(leftEye[0][1], leftEye[3][1]) + abs(leftEye[1][1] - leftEye[5][1]) * 1.5)
#                 cv2.rectangle(
#                     frame, (stratX, stratY), (endX, endY), (255, 0, 0), 1)

                # 绘制右眼边框
                stratX = int(
                    rightEye[0][0] - abs(rightEye[0][0] - rightEye[3][0]) * 0.3)
                stratY = int(
                    min(rightEye[0][1], rightEye[3][1]) - abs(rightEye[1][1] - rightEye[5][1]) * 1.5)
                endX = int(
                    rightEye[3][0] + abs(rightEye[0][0] - rightEye[3][0]) * 0.3)
                endY = int(
                    min(rightEye[0][1], rightEye[3][1]) + abs(rightEye[1][1] - rightEye[5][1]) * 1.5)
                # print stratX,stratY,endX,endY
#                 cv2.rectangle(
#                     frame, (stratX, stratY), (endX, endY), (255, 0, 0), 1)
                # print "人眼定位时间：{}".format(time.time()-t1)
                ...
                # 眨眼检测
                # 眼睛纵横比连续小于阈值的帧数eyeThreshold=0.24
                if ear < eyeThreshold:
                    eyeFrameNum += 1
                    # print "ear：{},eyeFrameNum:{}".format(ear,eyeFrameNum)

                # 如果EAR大于阈值
                else:
                    # print "ear：{},eyeFrameNum:{}".format(ear,eyeFrameNum)
                    # EAR小于阈值的总帧数大于等于CONSEC_FRAMES=3记一次眨眼
                    if eyeFrameNum >= EYE_CONSEC_FRAMES:
                        dA.BF += 1
                    # 重置EAR小于阈值的总帧数
                    eyeFrameNum = 0
                cv2.putText(frame, "Blinks: {}".format(
                    dA.BF), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                ############睁闭眼状态检测#################
                # 横纵比向量
                ...
                # print "人眼状态识别时间：{}".format(time.time()-t1)
                ################end########################
                ...
                # print "嘴部状态识别时间：{}".format(time.time()-t1)
                ####################end##############################
